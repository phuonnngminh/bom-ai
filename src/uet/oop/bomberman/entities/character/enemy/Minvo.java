...
public class Minvo extends Enemy {
    ...
    public void update() {
        ...
        if (bomber != null) {
            ...
            double distance = Math.sqrt(Math.pow(bomberX - minvoX, 2) + Math.pow(bomberY - minvoY, 2));

            // Kiểm tra xem khoảng cách giữa Minvo và Bomber có dưới một ngưỡng nhất định
            // hay không
            if (distance <= 3) {
                // Bomber ở gần, thực hiện hành động phù hợp ở đây
                // Ví dụ: Minvo tấn công Bomber hoặc tránh xa khu vực có Bomber
                movieTowardBomber();
                placeBomb();
            } else {
                // Bomber không ở gần, thực hiện hành động khác ở đây
                // Ví dụ: Minvo tiếp tục di chuyển theo hướng hiện tại
                _direction = _ai.calculateDirection();
                // move();
            }
        }
    ...
    private void movieTowardBomber() {
        Bomber bomber = _board.getBomber();
        if (bomber != null) {
            int bomberX = bomber.getXTile();
            int bomberY = bomber.getYTile();
    ...
    private void placeBomb() {
        ...
        if (Math.random() < bombRate) { // Kiểm tra xác suất đặt bom
            ...
            _board.addBomb(new Bomb(xt, yt, _board)); // Thêm bom vào bảng tại vị trí Minvo đang đứng
        }
    }
}
