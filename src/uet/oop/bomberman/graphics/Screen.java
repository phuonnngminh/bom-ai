...
public class Screen {
	...
	public void drawEndGame(Graphics g, int points) {
		...
		drawCenteredString("POINTS: " + points, getRealWidth(),
				getRealHeight() / 3 + (Game.TILES_SIZE * 2) * Game.SCALE, g);
		font = new Font("Arial", Font.PLAIN, 8 * Game.SCALE);
		g.setFont(font);
		g.setColor(Color.white);
		drawCenteredString("Press R to retry", getRealWidth(), getRealHeight() / 2 + (Game.TILES_SIZE * 4) * Game.SCALE,
				g);
		drawCenteredString("Press B to Back Home", getRealWidth(),
				getRealHeight() / 2 + (Game.TILES_SIZE * 5) * Game.SCALE, g);
	}
	...
	public void drawWinScreen(Graphics g, int points) {
		drawCenteredString("Press R to retry", getRealWidth(), getRealHeight() / 2 + (Game.TILES_SIZE * 4) * Game.SCALE,
				g);
		drawCenteredString("Press B to Back Home", getRealWidth(),
				getRealHeight() / 2 + (Game.TILES_SIZE * 5) * Game.SCALE, g);
